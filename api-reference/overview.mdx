...
All API endpoints should be secured using API key authentication. Zaymo will authenticate requests to your API using a custom header-based API key system. You should provide Zaymo with an API key that our system will include in all requests to your endpoints.

### API Key Implementation

Your API should validate the API key included in the `X-Zaymo-API-Key` header of all incoming requests from Zaymo:

```bash
X-Zaymo-API-Key: YOUR_PROVIDED_API_KEY
```

This authentication method is defined in our OpenAPI specification as `apiKeyAuth` and ensures secure communication between Zaymo's systems and your API endpoints.
...
- Return a `403 Forbidden` response if the API key exists but lacks permissions for the requested operation
...
1. **Header Check**: Verify the `X-Zaymo-API-Key` header exists
2. **Key Validation**: Confirm the API key matches your stored value
3. **Permission Check**: Ensure the key has access to the requested endpoint
4. **Request Processing**: Process the request if authentication succeeds
5. **Error Response**: Return appropriate error codes for authentication failures

Your API should validate the API key before processing any request data to ensure security.
